...
- `validate.yml` - BATS valid/negative fixtures for the bootc/native publication, runtime `/etc`, and duplicate-package guards + shellcheck + direct runtime-/etc-guard (`check-runtime-etc-guard.sh`) + native A/B static/contracts/publication-guard checks + bootc secure publication/artifact/policy/installer/update fixture contracts + expensive-workflow path-filter regression checks + mkosi summary validation on PRs. `test/publication-guards.bats` reuses the bootc guard's exhaustive mutation harness and isolates the other guards in temporary Git/contract fixtures; run it with `bats --tap ./test/publication-guards.bats`.
